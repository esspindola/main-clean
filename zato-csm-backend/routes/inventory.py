...
@router.get('')
def get_inventory(user=Depends(get_current_user), inventory_service = Depends(_get_inventory_service)):
    inventory = inventory_service.get_inventory()
    return {"success": True, "inventory": inventory}

@router.put('/{product_id}')
def update_inventory(product_id: int, quantity: int, request: Request, user=Depends(get_current_user), inventory_service = Depends(_get_inventory_service)):
    ...
    return {"success": True, "message": "Stock updated successfully", "inventory": result}