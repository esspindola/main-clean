...
class UserRepository(BaseRepository):
    ...
    def create_user(self, full_name: str, email: str, password: str, phone: str = None, address: str = None, role: str = "user", user_timezone: str = "UTC"):
        ...
        if self.db_type == 'mysql':
            with self._get_cursor() as cursor:
                cursor.execute(
                    ...
                    (full_name, email, password, phone, address, role, created_at, last_updated)
                )
                self.db.commit()
    ...
    def update_profile(self, user_id: int, updates: dict, user_timezone: str = "UTC"):
        ...
        with self._get_cursor() as cursor:
            cursor.execute("UPDATE users SET full_name=%s, phone=%s, address=%s, last_updated=%s WHERE id=%s",
                           (updates.get('full_name'), updates.get('phone'), updates.get('address'), updates.get('last_updated'), user_id))
            self.db.commit()
